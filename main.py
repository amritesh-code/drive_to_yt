...
def list_videos_in_folder(folder_id):
    ...
    page_token = None
    while True:
        resp = drive_service.files().list(q=query, fields="nextPageToken, files(id, name, mimeType, size)", pageToken=page_token, pageSize=1000).execute()
...
def main():
    ...
    for f in files:
        ...
        if fid in tracked:
            print("Skipping already uploaded:", name)
            continue
        ...
        try:
            ...
            tracked[fid] = {"name": name, "youtube_id": vid_id, "time": int(time.time())}
